...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
  ({ className, label, error, isValid, type = 'text', id, ...props }, ref) => {
    ...
    return (
      <div className="mb-4">
        {label && (
          <label className="block text-foreground-muted font-semibold mb-2" htmlFor={inputId}>
            {label}
        ...
        <div className="relative">
          <input
            ...
            id={inputId}
            className={cn(
              'w-full px-4 py-3 rounded-lg border bg-background text-foreground transition-all placeholder:text-foreground-muted',
