 ...
 *   node scripts/bundle-analyzer.mjs --json
 *   node scripts/bundle-analyzer.mjs --limit 500000
 */
  ...
  if (existsSync('.next')) {
    try {
      ...
      if (existsSync(buildManifestPath)) {
        ...
        // Fallback: check static files
        const staticPath = '.next/static';
        if (existsSync(staticPath)) {
          // Simple size calculation for static assets
...
function getRecommendations(sizes) {
  ...
  for (const [page, size] of Object.entries(sizes)) {
    if (page !== 'total' && page !== 'static' && page !== 'dist' && size > 200000) { // 200KB per page
      recommendations.push({
        severity: 'medium',
